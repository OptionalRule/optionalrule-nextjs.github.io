...
describe('buildRelationshipGraph', () => {
  it('returns a populated entity inventory and an empty edge list when no rules match', () => {
    ...
    const graph = buildRelationshipGraph(minimalInput(), { tone: 'balanced', gu: 'normal', distribution: 'realistic', settlements: 'normal' }, [], rng)
  ...
  it('produces deterministic output for the same seed and input', () => {
    const a = buildRelationshipGraph(minimalInput(), { tone: 'balanced', gu: 'normal', distribution: 'realistic', settlements: 'normal' }, [], createSeededRng('graph-test-4'))
    const b = buildRelationshipGraph(minimalInput(), { tone: 'balanced', gu: 'normal', distribution: 'realistic', settlements: 'normal' }, [], createSeededRng('graph-test-4'))
    expect(a).toEqual(b)
  })

  it('produces a HOSTS edge when a settlement has a bodyId', () => {
    const rng = createSeededRng('graph-test-hosts')
    const graph = buildRelationshipGraph(inputWithHostedSettlement(), { tone: 'balanced', gu: 'normal', distribution: 'realistic', settlements: 'normal' }, [], rng)
  ...
  it('produces deterministic edges and historicalEdgeIds for the same seed', () => {
    const a = buildRelationshipGraph(
      ...
      { tone: 'balanced', gu: 'normal', distribution: 'realistic', settlements: 'normal' },
      controlsFacts(),
