...
|---|---|---|---|---|
| **A** | Add `BuildGraphOptions` (NEW interface in `graph/types.ts`), thread it through `buildRelationshipGraph` alongside `EntityInventoryInput`. Expand `NAMED_KINDS` in `score.ts` to include `phenomenon` + `guHazard` so DESTABILIZES can reach the spine. Implement `toneMultiplier(tone, edgeType)` and `isSpineEligibleForGu(edge, gu)`. Matrix snapshot test pinning STRUCTURAL fields only (no prose strings). Audit check `narrative.spineToneSensitivity`. | ~5 days | Tone weights actually fire (no longer dead letters on phenomenon-anchored edges). `gu='fracture'` widens spine pool. `tone='balanced'` + `gu='normal'` byte-identical to pre-feature. | ✅ Done — [plan](./NARRATIVE_GRAPH_TONE_GU_PHASE_A_PLAN.md) (sub-fork fired: 0/200 baseline survey triggered authoring of 3 phenomenon-anchored DESTABILIZES rules; post-fork 200/200; phase6On.test.ts snapshots regenerated as the third deliberate softening of the snapshot contract; matrix shows 2 distinct spine edge types with intended tone-conditioned variation; deep audit 0 errors / 0 warnings) |
| **B** | Author per-tone faction-name stem + suffix banks. Implement deterministic `generateFactions(seed, tone, count)` in `lib/generator/factions/` (NEW module). Wire into per-system construction at `lib/generator/index.ts:2789` (replace static `namedFactions` load). Migrate or remove `data/narrative.json:namedFactions[]`. Per-system faction-cohesion test (no mixing dramatic/clinical names). Audit: ≥100 unique faction names across 4800 systems (today: 10). | ~5–7 days | Per-tone factions drawn from 100s of stems instead of the 10-card shared deck. The "Kestrel/Red Vane everywhere" complaint dissolves at the corpus level. `proseUnchanged.test.ts` regenerated — second deliberate softening of the byte-identical-default contract. | ✅ Done — [plan](./NARRATIVE_GRAPH_TONE_GU_PHASE_B_PLAN.md) (deep audit: 330 unique faction names across 4800 systems; 0 errors / 0 warnings; proseUnchanged snapshot stayed byte-identical because the 3 seeds it samples don't render faction names — the predicted second softening did not materialize) |
| **C** | Author per-tone era pools (3 tones × 8–12 eras). Per-tone body[0] template variants for top-4 spine-eligible edge types (CONTESTS, CONTROLS, DESTABILIZES, one of {CONTRADICTS, BETRAYED}): ~36–48 strings. Per-tone spineSummary variants for the same 4 types: ~24–36 strings. Per-tone connective banks in `connectives.ts`. Renderer wiring in `renderClause` / `renderSpineSummary`. Cross-tone snapshot test for the top-4 edge types showing visibly different prose voice. | ~6–8 days | Even when two systems pick the same edge type for the spine, body[0] reads in a different voice (cinematic vs astronomy vs balanced). The Phase 7 "every system reads template-shuffled" complaint is resolved. | ✅ Done — [plan](./NARRATIVE_GRAPH_TONE_GU_PHASE_C_PLAN.md) (task 1 frequency survey: post-Phase-A only DESTABILIZES + CONTESTS fire as spine across 600 seeds — CONTROLS appeared 2/600, CONTRADICTS/BETRAYED never; scope shrunk from top-4 to top-2 edge types because per-tone variants for non-firing types would create dead-letter strings; 28 strings authored across the 2 firing types × 3 surfaces × 2 non-balanced tones; per-tone era pools shipped; per-tone connectives shipped; renderer wired through `BuildGraphOptions` chain; spineToneVoiceMatrix.test.ts pins the 3-cell tone matrix; `narrative.body0VoiceDiversity` audit check live (deep-audit corpus passes); proseUnchanged snapshot stayed byte-identical because the 3 sampled seeds use balanced tone with single-element fallback variants — predicted third softening did NOT materialize; deep audit 0 errors / 0 warnings) |
| **D** | Extend `BuildGraphOptions` to carry `distribution` + `settlements`. Distribution-axis weights in `INPUT_WEIGHTS` (frontier boosts `visibility: 'contested'`; realistic boosts `visibility: 'open'`). Density-conditioned cluster pulling in `clusters.ts` (hub pulls more CONTROLS/DEPENDS_ON; sparse drops multi-faction edges). Matrix snapshot extension: 16 representative cells (diagonal subset of the 96 full Cartesian product). | ~3–4 days | All four input axes produce distinguishable output. The PHASE_7_SAMPLE_REVIEW.md "20-sample review reads as 20 distinct systems" bar is achieved. | ✅ Done — [plan](./NARRATIVE_GRAPH_TONE_GU_PHASE_D_PLAN.md) (BuildGraphOptions now carries all 4 axes; distribution-axis weights ship 6 numbers (2 distributions × 3 visibility tiers); density-conditioned cluster pulling adds sparse-drop and hub-pull rules; 16-cell axis matrix snapshot covers the diagonal of the 96-cell Cartesian product; phase6On.test.ts and spineToneGuMatrix.test.ts snapshots regenerated as the fourth deliberate softening of the snapshot contract — proseUnchanged stayed byte-identical for the 3 sampled seeds; deep audit 0 errors / 0 warnings on `narrative.distributionAxisSensitivity` + `narrative.densityAxisSensitivity`) |
