...
- Companion-debris configuration layer: ten archetypes (mass-transfer stream, common-envelope shell, polar circumbinary ring, binary Trojan camp, hierarchical inner-pair halo, Kozai-Lidov scattered halo, Hill-sphere capture cone, resonance-pumped exocomet swarm, accretion bridge, Gardener-quarantine cordon) derived deterministically from companion mode, mass ratio, and stellar activity. Each spawns a paired structured `SystemPhenomenon` with full transit/survey/conflict/anchor beats from authored prose pools. Settlements and ruins can anchor on compatible fields (`embedded`, `edge-only`, `transient-only`, `unanchorable`). Surfaces on BodyDetailPanel, OrbitalTable, Markdown/JSON exports, and four new 3D-viewer renderers (ring, shell, stream, halo) plus five legend chips. Bundled with a Phase 0 encroachment fix that closed the orbital-sibling mass-arg swap bug and four viewer rendering caps (sub-system extent, moon envelope, clearance inflation, circumbinary keep-out ring sync). See `docs/superpowers/specs/2026-05-15-companion-systems-design.md`.
