...
*The edge of the torch defined the edge of the world.* The hobby owes much of its tension and excitement to that simple design from the early days.
...
Here are some options to make light and darkness matter again at your table. The first section works entirely within 5e's existing rules. The second section offers house rules that amplify the effect further. Pick and choose what fits your table's style. Also, by Elminster's Fluffy Beard, talk to your players about changes or rules you're thinking about adopting. When the table feels like this is all part of a shared vision, everyone has more fun.

### Working Within 5e: No House Rules Required

These tools work entirely within 5e's existing rules or require only procedural changes at the table. You don't need session zero buy-in or player approval to start using them. They're techniques and practices you can implement today. Pick two or three and commit to them for a full dungeon, then add more once your table sees the difference.
...
* Apply the consequences. Dim light worsens sight-based Perception. Darkness blinds creatures without help. Creatures attacking a blinded target have advantage, and the blinded creature attacks at disadvantage. Suddenly, an enemy taking the Hide action
* Use darkvision as written, don't handwaive it. It turns darkness into dim light within its radius, which still imposes those Perception penalties. It does not grant color, depth, or detail suitable for reading, tracking faint stains, or spotting hairline cracks. When players say "I have darkvision so I see fine," answer with "You are able to see a bit but poorly. Things are time and difficult to make out."

#### 2. Put time back on the table

* Choose a simple clock. A torch or lantern is one hour or six ten-minute turns.
* Charge time for meaningful actions. Moving to a new area, mapping, searching carefully, forcing a stuck door, or looting a room consumes a turn. State the new time and remaining light. *Turns are a useful tool, use them.*
* Tie risk to time. Roll for some danger or event every turn or two. 
* Leans into real-time when convienent.  If the party debates for 10 minutes of real time, then mark off a turn in game. In addition to tension, it keeps things moving along.

#### 3. Let the dungeon fight the flame

* Telegraph risk. "The tunnel breathes with a steady draft. Your torch sputters." Add a few environmental risks like mud, wind, or dripping water that could snuff the light. Always have a monster lurking in the dark ready to take advantage.
* Aim attacks and hazards at the light. Goblins carry sloshing buckets. Bats swarm toward flame. A trapped door exhales and threatens to snuff exposed torches unless covered. The Disarm Action can suddenly become very powerful against a light source.
...
#### 5. Stock better light and plan to protect it
...
#### 6. Use maps and space to make darkness do work
...
#### 7. Script simple behaviors for dark-native foes
...
#### 8. Make rulings that keep decisions sharp
...
### House Rules for Maximum Tension

The tools above work great within RAW 5e and will already add substantial tension to your dungeon delves. But if you want to dial things up even further, the following house rules are optional upgrades that amplify everything in the previous section. These modify core character abilities, so you'll need to discuss them at session zero and get player buy-in. Even implementing just one of these changes the game significantly.

#### 1. Rebalance darkvision without feeling punitive
...
#### 2. Diversify senses so the party feels special without free flashlights
...
Steal these cards for your table rules section. Use the first set immediately, add the second after session zero discussion.

### Quick Reference: RAW Procedures
